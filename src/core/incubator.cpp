...
void QsIncubationController::initLoop() {
	...
	if (animationDriver) {
		...
		qCCritical(
		    logIncubator
		) << "Render loop does not have animation driver, can't hook animationStopped.";
	}
}
...
void QsIncubationController::incubate() {
	if (this->renderLoop && this->incubatingObjectCount()) {
		if (!this->followRenderloop) {
			this->incubateFor(10);
			if (this->incubatingObjectCount()) this->incubateLater();
		} else if (!this->followRenderloop && this->renderLoop->interleaveIncubation()) {
...
void QsIncubationController::incubatingObjectCountChanged(int count) {
	if (count && this->renderLoop
	    && (this->followRenderloop || !this->renderLoop->interleaveIncubation()))
	{
		this->incubateLater();
	}
}
