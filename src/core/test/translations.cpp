...
void TestTranslations::catalogAndLanguageSwitch_data() { // NOLINT(readability-identifier-naming)
	QTest::addColumn<bool>("withFallback");
	QTest::newRow("without fallback") << false;
	QTest::newRow("with source-language fallback") << true;
}

void TestTranslations::catalogAndLanguageSwitch() {
	QFETCH(const bool, withFallback);
	...
	if (withFallback) {
		QVERIFY(QFile::copy("qml_en.qm", root.filePath("i18n/qml.qm")));
	}
	...
	environment.insert("QS_TEST_FALLBACK", withFallback ? "1" : "0");
	environment.insert("QS_DISABLE_FILE_WATCHER", "1");
	environment.insert("QS_NO_RELOAD_POPUP", "1");
	...
	QVERIFY2(!output.contains("QApplication::removeTranslator"), output.constData());
}
